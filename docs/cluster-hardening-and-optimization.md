...
- [ ] **1.1** [S] Prefill-guard vote-on-any-exception: catch `Exception` in
  `check_collective`'s local check, vote 1, re-raise locally after the
  collective (`prefill_guard.py:185-231`). (§A2)
- [ ] **1.2** [S] Planner fail-closed TP gate for biased/latent nemotron_h
...
- [ ] **1.3** [S] Snapshot supervisor state under the condition lock in
  `status()` (`launch.py:2194-2210`). (§C2)
- [ ] **1.4** [S] Marker-read-first health check: drop the redundant
