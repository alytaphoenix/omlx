...
class EnginePool:
    ...
    async def _load_engine(
        ...
        try:
            ...
            engine = None
            # The helper owns the routing policy: a registered deployment for
            # a "batched" entry, or for a "vlm" entry whose deployment is an
            # explicit text-only opt-in. Every other engine type gets None,
            # so local dispatch below is unchanged for them.
            deployment = self._distributed_deployment_for_entry(entry)
