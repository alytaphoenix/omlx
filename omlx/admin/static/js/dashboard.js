    ...
    function dashboard() {
        return {
            ...
            clusterParallelismChoices() {
                ...
                if (nodes <= 1) {
                    return [
                        { size: 1, label: window.t('cluster.plan.pipeline_only'), supported: true, reason: '' },
                    ];
                }
                ...
                const choices = [
                    {
                        size: 1,
                        label: window.t('cluster.plan.pipeline_only'),
                        supported: support.pipeline.supported,
                        reason: support.pipeline.reason,
                    },
                    {
                        size: nodes,
                        label: window.t('cluster.plan.way_tensor').replace('{size}', nodes),
            ...
            clusterParallelismHint() {
                ...
                return Number(this.clusterPlanTensorParallelSize) > 1
                    ? window.t('cluster.plan.every_accelerator_works')
                    : window.t('cluster.plan.each_accelerator_holds_layers').replace('{count}', this.clusterPlanNodes.length);
            },
            ...
            clusterStartButtonLabel() {
                ...
                if (this.clusterLinkSetupLoading) {
                    return this.clusterActivationProgress
                        || window.t('cluster.planner.configuring_thunderbolt');
                }
                if (this.clusterActivationLoading) {
                    return this.clusterActivationProgress || window.t('cluster.planner.starting_cluster');
                }
                if (this.clusterAutoconfigureLoading) {
                    return window.t('cluster.planner.checking_every_worker');
                }
                return window.t('cluster.planner.start_cluster');
            },
